...
def CheckOverlapCondition(idx0, idx1, R_det, source_pos, fsm, mrot, proj_spacing, proj_size, proj_origin, proj_direction):
    rp0, rp1, volDirection, volSpacing = ComputeRetroProj(idx0, idx1, R_det, source_pos, fsm, mrot, proj_spacing, proj_size, proj_origin, proj_direction)
...
def ComputeRetroProj(idx0, idx1, R_det, source_pos, fsm, mrot, proj_spacing, proj_size, proj_origin, proj_direction):
...
class DCCWithBPinAnAcquisition():
    def __init__(self, geometry_array, source_pos_array,
        ...
        if self.n_rotation == 1:
            ...
            self.d = self.geometry[8, self.n_proj_per_rotation-1] - self.geometry[8, 0]
        self.R_fov = self.geometry[0, 0]*np.sin(self.proj_size[0]*self.proj_spacing[0]/(2*self.geometry[1, 0]))  # ANGLE EN RADIAN
        self.P = np.abs(self.d*self.geometry[1, 0]/(self.geometry[0, 0]*self.proj_size[1]*self.proj_size[1]))
    ...
    def CheckPairGeometry(self, idx0, idx1):
        ...
        if np.cos(self.geometry[2, idx1]-self.geometry[2, idx0]) > 2*((self.R_fov)/self.geometry[0, 0])**2-1:
            if CheckOverlapCondition(idx0, idx1, self.geometry[9, 0], self.source_pos, self.fsm, self.mrot, self.proj_spacing, self.proj_size, self.proj_origin, self.proj_direction):
                return idx1-idx0
            else:
                return 0
        else:
